...
Begin
	...
	result <= in1 WHEN ctl ='1' ELSE in0 ; 
...
architecture circuits of bshift is
  ...
  signal bshift_in2s : std_logic_vector(1 downto 0);
  signal bshift_in4s : std_logic_vector(3 downto 0);
  signal bshift_in8s : std_logic_vector(7 downto 0);
  signal bshift_in16s : std_logic_vector(15 downto 0);
...
begin  -- circuits
  ...
  R1w:  R1s <= '0' & bshift_in(31 downto 1); -- just wiring
  R1m:  mux_32 port map (in0=>bshift_in, in1=>R1s, ctl=>shift(0), result=>R1);
  ...
  A1w:  A1s <= bshift_in(31)&bshift_in(31 downto 1); -- just wiring
  A1m:  mux_32 port map (in0=>bshift_in, in1=>A1s, ctl=>shift(0), result=>A1);
  A2w:  A2s <= bshift_in2s&A1(31 downto 2);    -- just wiring
  A2m:  mux_32 port map (in0=>A1, in1=>A2s, ctl=>shift(1), result=>A2);
  A4w:  A4s <= bshift_in4s&A2(31 downto 4);    -- just wiring
  A4m:  mux_32 port map (in0=>A2, in1=>A4s, ctl=>shift(2), result=>A4);
  A8w:  A8s <= bshift_in8s&A4(31 downto 8);    -- just wiring
  A8m:  mux_32 port map (in0=>A4, in1=>A8s, ctl=>shift(3), result=>A8);
  A16w: A16s <= bshift_in16s&A8(31 downto 16); -- just wiring
  A16m: mux_32 port map (in0=>A8, in1=>A16s, ctl=>shift(4), result=>A16);
  AS2:  bshift_in2s <= bshift_in(31) & bshift_in(31);  -- just wiring
  AS4:  bshift_in4s <= bshift_in2s & bshift_in2s;      -- just wiring
  AS8:  bshift_in8s <= bshift_in4s & bshift_in4s;      -- just wiring
  AS16: bshift_in16s <= bshift_in8s & bshift_in8s;     -- just wiring
  OLR:  mux_32 port map (in0=>R16, in1=>L16, ctl=>left, result=>LR);
  LOG:  mux_32 port map (in0=>A16, in1=>LR, ctl=>logical, result=>bshift_out);
